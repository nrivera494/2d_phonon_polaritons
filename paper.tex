...
\begin{equation}
\Pi_{qq}(\mathbf{q},\omega) = \frac{1}{\epsilon_0\hbar\Omega} \frac{2\omega_{\mathbf{q}L}}{\omega^2_{\mathbf{q}TO}-\omega^2}|\hat{q}\cdot\langle 1_{\mathbf{q}L}|\mathbf{P}(\mathbf{q})|0_{\mathbf{q}\sigma}\rangle|^2.
\end{equation}
In this expression, $\omega_L$ and been re-named as $\omega_{TO}$, because in the absence of LO-TO splitting, they are degenerate. Additionally, we have defined eigendisplacements $\boldsymbol{\eta}_{\kappa\mathbf{q}}\equiv \frac{e_{\kappa\mathbf{q},L}}{\sqrt{M_{\kappa}}}$, where $e_{\kappa\mathbf{q},L}$ is the unit-normalized polarization vector of atom $\kappa$ in the unit cell oscillating according to a longitudinal phonon of wavevector $\mathbf{q}$ and $M_{\kappa}$ is the mass of atom $\kappa$.

As is well known, the zeros of the RPA dielectric function correspond to longitudinal electromagnetic waves. Thus, the condition for the zeros of the dielectric function correspond to
\begin{equation}
\omega^2_{\mathbf{q}} - \omega^2_{\mathbf{q},T} = \frac{e^2}{2\epsilon_0\epsilon_{\mathrm{env}}q}\frac{1}{\Omega}q^2\Big|\sum\limits_{\kappa,j}Z_{\kappa,ij}\eta_{\kappa,j}  \Big|^2
\end{equation}
