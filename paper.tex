...
\end{equation}
where $m,n$ are states of the phononic Fock space of the monolayer, $P_{i,mn} = \sum\limits_{\kappa,ij}\langle m | u_{\kappa,j} | n \rangle$ are matrix elements of the polarization associated with phonon modes, $E_{m (n)}$ is the energy of state $m (n)$, and $\beta$ is the inverse temperature. Evaluating the contribution of optical phonons in the harmonic approximation to the polarization-polarization response, one finds that the response function is given by: 
\begin{equation}
...
\end{equation}
We consider a solution of the form $H(z) = e^{-\kappa_1 z}$ for $z > 0$ with $\kappa_1 = \sqrt{q^2-\epsilon_1\frac{\omega^2}{c^2}}$ and a solution of the form $H(z) = -ce^{\kappa_2 z}$ for $z < 0$ with $\kappa_2 = \sqrt{q^2-\epsilon_2\frac{\omega^2}{c^2}}$. Continuity of the displacement field across the interface forces $c = \frac{\epsilon_1}{\epsilon_2}$. Meanwhile, the boundary condition on the magnetic field is $\unitvec{z}\times(\mathbf{H}^{(+)}-\mathbf{H}^{(-)}) = \mathbf{K} = -\iu\omega\mathbf{P}_{\mathrm{s}}$ where $\mathbf{K}$ is the surface current density, expressed through the surface polarization density $\mathbf{P}_{\mathrm{s}}$. 

The condition for an evanescent electromagnetic wave can be written as $\epsilon_{\mathrm{RPA}}(\omega) = 0$, where $\epsilon_{\mathrm{RPA}}(\omega) = \epsilon_{\mathrm{env}}+\frac{1}{2}q\Pi(\mathbf{q},\omega)$ coincides with the phonon contribution to the longitudinal dielectric function of the monolayer in the random-phase approximation. 
